...
import { setOptionsBasedOnType } from '../handlers/roleMenu.js';
...
export async function execute(
    ...
    try {
        ...
        const buttonRow = newButtonRow(mainAction);
            ...
            if (mainAction === 'roles') {
                if (secondaryAction === 'userRoles') {
                    ...
                    if (stage === 'startMessage') {
                        if (anythingElse.includes('back'))
                            ...
                            log(
                                { level: 'log' },
                                `${interaction.user.tag} created a new role system interaction`,
                            );
                            await interaction.deferReply({ ephemeral: true });
                        }
                        const selectRoleTypeEmbed = newEmbed(
                            'User Roles',
                            stripIndent`
                        ...
                        const roleCategoryMenu =
                            new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
                                [
                                    new StringSelectMenuBuilder()
                                        .setCustomId(
                                            `${mainAction}|${secondaryAction}|selectRoleBasedCategory`,
                                        )
                                        .setOptions(Constants.roleTypes)
                        ...
                        buttonRow.setComponents([
                            new ButtonBuilder()
                                .setCustomId(
                                    `${mainAction}|${secondaryAction}|startMessage|back`,
                                )
                                .setLabel('Back')
                        ...
                        const selectRolesBasedOnType = new EmbedBuilder()
                            ...
                            You have selected ${uppercaseWord(
                                selectedRoleType,
                            )}!

                            Use the menu below to select the role${(() => {
                                if (
                                    selectedRoleType === 'pronouns' ||
                                    selectedRoleType === 'pings'
                                )
                                    return 's';
                                else return '';
                            ...
                            `);
                        const selectRoleMenu = newStringSelectMenuBuilderRow(
                            mainAction,
                            secondaryAction,
                            'selectedRoles',
                            selectedRoleType,
                        ...
                        if (selectedRoleType === 'age') {
                            selectRoleMenu.components[0]
                                .setOptions(Constants.ageRoles)
                                .setPlaceholder('Select one role.');
                        } else if (selectedRoleType === 'sexuality') {
                            selectRoleMenu.components[0]
                                .addOptions(Constants.sexualityRoles)
                                .setPlaceholder('Select one role.');
                        } else if (selectedRoleType === 'pronouns') {
                            selectRoleMenu.components[0]
                                .addOptions(Constants.pronounRoles)
                                .setMinValues(1)
                                .setMaxValues(Constants.pronounRoles.length)
                            ...
                            selectRoleMenu.components[0]
                                .addOptions(Constants.pingRoles)
                                .setMinValues(1)
                                .setMaxValues(Constants.pingRoles.length);
                        } else {
                            log(
                                { level: 'error' },
                                `Unknown category was received, got ${selectedRoleType}`,
                            );
                        }
                        return await interaction.editReply({
                            content: '',
                    ...
                    if (stage === 'removeRole') {
                        ...
                        const fetchedRoles = interaction.guild.roles.cache;
                        ...
                        try {
                            ...
                            if (
                                ...
                                roleType = 'pronouns';
                            } else if (colorRoles.includes(roleToRemove.name)) {
                                roleType = 'color';
                            } else if (
                                [
                                    'all pings',
                                ...
                                log(
                                    { level: 'error' },
                                    `Received role didn't match any type, got ${roleToRemove.name}`,
                                );
                            }
                            ...
                            let previousTypeIndex =
                                roleTypes.indexOf(roleType) - 1;
                    ...
                    if (stage === 'removeRoles') {
                        ...
                        const rolesToRemove = fetchedRoles.filter((role) =>
                        ...
                        try {
                            ...
                            if (
                                ...
                                colorRoles.includes(rolesToRemove.first().name)
                            ) {
                                roleType = 'color';
                            } else if (
                                [
                                ...
                                log(
                                    { level: 'error' },
                                    `The selected role for removal didn't match any type, got ${rolesToRemove
                                        .first()
                                        .name.toLowerCase()}`,
                                );
                            }
                ...
                `,
                    ...
                    if (stage === 'updateRoles') {
                        ...
                        const rolesToRemove = rolesToCheck.filter((role) =>
                            memberRoles.cache.has(role.id),
                        );
                        const rolesToAdd = rolesToCheck.filter(
                            (role) => !memberRoles.cache.has(role.id),
                        );
                        try {
              ...
              `,
                            ...
                            .addFields([
                                {
                                    name: 'Added Roles',
                                    value: `${mentionAddedRoles}`,
                                    inline: true,
                                },
                                {
                                    name: 'Removed Roles',
                                    value: `${mentionRemovedRoles}`,
                                    inline: true,
                                },
                            ]);
                        let roleType: string = '';
                        if (
                            ...
                            colorRoles.includes(rolesToCheck.first().name)
                        ) {
                            roleType = 'color';
                        } else if (
                            [
                            ...
                            console.log(
                                `the role to check didn't match any type???? got ${rolesToCheck
                                    .first()
                                    .name.toLowerCase()}`,
                            );
                        }
                    ...
                    if (stage === 'onlyAddRoles') {
                        ...
                        const fetchedRoles =
                            await interaction.guild.roles.fetch();
                        const rolesToAdd = fetchedRoles.filter((role) =>
                            roleNames.includes(role.name),
                        );
                        try {
                            await memberRoles.add(rolesToAdd);
                        ...
                        if (
                            ...
                            colorRoles.includes(rolesToAdd.first().name)
                        ) {
                            roleType = 'color';
                        } else if (
                            [
                ...
                if (mainAction === 'roles') {
                    if (secondaryAction === 'userRoles') {
                        if (stage === 'selectRoleBasedCategory') {
                            ...
                            const selectRolesBasedOnType = newEmbed(
                                uppercaseWord(selectedRoleType),
                                stripIndent`
										You have selected ${uppercaseWord(selectedRoleType)}!
                            ...
                            selectRoleMenu.components[0].setOptions(
                                setOptionsBasedOnType(selectedRoleType),
                            );
                            return await interaction.editReply({
                                embeds: [selectRolesBasedOnType],
                                components: [selectRoleMenu, buttonRow],
                            ...
                            if (selectedRoleType === 'age') {
                                const rolesToCheck = fetchedRoles.filter(
                                    (role) =>
                                        ['minor', 'boomer'].includes(
                                ...
                                colorRoles.forEach((color, index) => {
                                    colorRoles[index] = color.toLowerCase();
                                });
                                // console.log(colors);
                                const rolesToCheck = fetchedRoles.filter(
                                    (role) =>
                                ...
                                (() => {
                                    selectedRoles.forEach(
                                        (selectedRoleName) => {
                                            ...
                                            if (!found) {
                                                console.log(
                                                    `${selectedRoleName} was not found within the server's roles.`,
                                                );
                                                difference.push(
                                                    selectedRoleName,
                                                );
                        ...
                        `,
                            ...
                            /* Only Additions */
                            if (
                                rolesMarkedForRemoval.size === 0 &&
                                rolesMarkedForAddition.size !== 0
                            ) {
                                let roleType: string = '';

                                if (
                                    ['minor', 'boomer'].includes(
                                        rolesMarkedForAddition
                                            .first()
                                            .name.toLowerCase(),
                                    )
                                ) {
                                    ...
                                    colorRoles.includes(
                                        rolesMarkedForAddition.first().name,
                                    )
                                ) {
                                    roleType = 'color';
                                } else if (
                                    ...
                                    console.log(
                                        `the role to add didn't match any type???? got ${rolesMarkedForAddition
                                            .first()
                                            .name.toLowerCase()}`,
                                    );
                                }
                                ...
                                const rolesAddedEmbed = new EmbedBuilder()
                                    ...
                                    .setTimestamp();
                                try {
                                    await memberRoles.add(
                                        rolesMarkedForAddition,
                                        'Roles were selected by the user.',
                                    );
                                } catch (err) {
                                    console.error(err);
                                }
                  ...
                  `);
                                    buttonRow.setComponents([
                                        new ButtonBuilder()
                                            .setCustomId(
                                                `${mainAction}|${secondaryAction}|removeRoles|${roleNames}`,
                                            )
                  ...
                  `);
                                ...
                                buttonRow.setComponents([
                                    ...
                                    new ButtonBuilder()
                                        .setCustomId(
                                            `${mainAction}|${secondaryAction}|onlyAddRoles|${roleNamesToAdd}`,
                                        )
                            ...
                            if (
                                ...
                                console.log(
                                    'WHAT HOW. no addition AND no removals wth',
                                );
                                return await interaction.update({
                                    content:
                                        '(The bot encountered an error, please try again later <3)',
                                });
                            }
                        }
                    }
