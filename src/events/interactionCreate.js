...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, StringSelectMenuBuilder, userMention, Collection, roleMention, } from 'discord.js';
    ...
    try {
        if (interaction.isCommand()) {
            if (interaction.isChatInputCommand()) {
                ...
                if (command === undefined) {
                    console.log(`The command you tried to execute doesn't exist, how? got ${interaction.commandName}`);
                    return;
                }
        ...
        if (interaction.isButton()) {
            ...
            `)
                        ...
                        const roleCategoryMenu = new ActionRowBuilder().addComponents([
                        ...
                        const selectRolesBasedOnType = new EmbedBuilder()
                            .setColor('Random')
                            .setTitle(selectedRoleType.replace(selectedRoleType.charAt(0), selectedRoleType.charAt(0).toUpperCase())).setDescription(stripIndent `
            You have selected ${selectedRoleType.replace(selectedRoleType.charAt(0), selectedRoleType.charAt(0).toUpperCase())}!
            ...
            `);
                        const selectRoleMenu = new ActionRowBuilder().addComponents([
                            new StringSelectMenuBuilder().setCustomId(`${mainAction}|${secondaryAction}|selectedRoles|${selectedRoleType}`),
                        ]);
                        if (selectedRoleType === 'age') {
                            ...
                            colors.forEach((color) => colorOptions.push({
                                label: color,
                            ...
                            selectRoleMenu.components[0]
                                ...
                                {
                                    label: 'SMP Pings',
                                    description: 'You will be pinged whenever any SMP-Related activity / announcement is made.',
                                    value: 'smp member',
                                },
                                // {
                                //   label: 'Game Pings',
                                ...
                                {
                                    label: 'VC Pings',
                                    description: 'You will be pinged whenever a member is looking for someone to VC with.',
                                    value: 'vc pings',
                                },
                            ])
                                .setMinValues(1)
                    ...
                    if (stage === 'removeRole') {
                        ...
                        try {
                            ...
                            if (['minor', 'boomer'].includes(roleToRemove.name.toLowerCase())) {
                            ...
                            else if (['he/him', 'she/her', 'they/them', 'any pronouns', 'ask me for pronouns'].includes(roleToRemove.name.toLowerCase())) {
                                roleType = 'pronouns';
                            }
                                ...
                                console.log(`the role to remove didn't match any type???? got ${roleToRemove.name}`);
                            }
                ...
                `)
                            ...
                            buttonRow.setComponents([
                                (() => {
                                    if (nextTypeIndex === roleTypes.length)
                                        nextTypeIndex = 0;
                                    return new ButtonBuilder()
                                        .setCustomId(stripIndent `${mainAction}|${secondaryAction}|selectRoleBasedCategory|${roleTypes.at(nextTypeIndex)}`)
                                        .setLabel(stripIndent `Next: ${roleTypes
                                        .at(nextTypeIndex)
                                        .replace(roleTypes.at(nextTypeIndex).charAt(0), roleTypes.at(nextTypeIndex).charAt(0).toUpperCase())}`)
                                        .setStyle(ButtonStyle.Success);
                                })(),
                                (() => {
                                    return new ButtonBuilder()
                                        .setCustomId(stripIndent `${mainAction}|${secondaryAction}|selectRoleBasedCategory|${roleTypes.at(previousTypeIndex)}`)
                                        .setLabel(stripIndent `Previous: ${roleTypes
                                        .at(previousTypeIndex)
                                        .replace(roleTypes.at(previousTypeIndex).charAt(0), roleTypes.at(previousTypeIndex).charAt(0).toUpperCase())}`)
                    ...
                    if (stage === 'removeRoles') {
                        ...
                        try {
                            ...
                            if (['minor', 'boomer'].includes(rolesToRemove.first().name.toLowerCase())) {
                            ...
                            else if (['he/him', 'she/her', 'they/them', 'any pronouns', 'ask me for pronouns'].includes(rolesToRemove.first().name.toLowerCase())) {
                                roleType = 'pronouns';
                            }
                                ...
                                console.log(`the role to remove didn't match any type???? got ${rolesToRemove
                                    .first()
                                    .name.toLowerCase()}`);
                            }
                            const roleTypes = ['age', 'sexuality', 'pronouns', 'color', 'pings'];
                ...
                `)
                    ...
                    if (stage === 'updateRoles') {
                        ...
                        const mentionAddedRoles = [];
                        const mentionRemovedRoles = [];
              ...
              `)
                        ...
                        else if (['he/him', 'she/her', 'they/them', 'any pronouns', 'ask me for pronouns'].includes(rolesToCheck.first().name.toLowerCase())) {
                            roleType = 'pronouns';
                        }
                        else if (colors.includes(rolesToCheck.first().name)) {
                            roleType = 'color';
                        }
                        else if (['all pings', 'smp member', 'poll pings', 'announcement pings', 'vc pings'].includes(rolesToCheck.first().name.toLowerCase())) {
                            roleType = 'pings';
                        }
                        else {
                            console.log(`the role to remove didn't match any type???? got ${rolesToCheck
                                .first()
                                .name.toLowerCase()}`);
                        }
                        const roleTypes = ['age', 'sexuality', 'pronouns', 'color', 'pings'];
                    ...
                    if (stage === 'onlyAddRoles') {
                        ...
                        else if (['he/him', 'she/her', 'they/them', 'any pronouns', 'ask me for pronouns'].includes(rolesToAdd.first().name.toLowerCase())) {
                            roleType = 'pronouns';
                        }
                        else if (colors.includes(rolesToAdd.first().name)) {
                            roleType = 'color';
                        }
                        else if (['all pings', 'smp member', 'poll pings', 'announcement pings', 'vc pings'].includes(rolesToAdd.first().name.toLowerCase())) {
                            roleType = 'pings';
                        }
                        else {
                            console.log(`the role to remove didn't match any type???? got ${rolesToAdd
                                .first()
                                .name.toLowerCase()}`);
                        }
                        const roleTypes = ['age', 'sexuality', 'pronouns', 'color', 'pings'];
            ...
            `);
                            ...
                            if (selectedRoles.length === 0)
                                return console.log('no roles wtf?');
                            if (!interaction.inCachedGuild())
                                return;
                            const memberRoles = interaction.member.roles;
                            let fetchedRoles = await interaction.guild.roles.fetch();
                            let rolesMarkedForRemoval = new Collection();
                            let rolesMarkedForAddition = new Collection();
                            let rolesReplaced = new Collection();
                            if (selectedRoleType === 'age') {
                                const rolesToCheck = fetchedRoles.filter((role) => ['minor', 'boomer'].includes(role.name.toLowerCase()));
                                ...
                                // console.log(colors);
                                const rolesToCheck = fetchedRoles.filter((role) => colors.includes(role.name.toLowerCase()));
                                rolesToCheck.forEach(async (role) => {
                                    if (memberRoles.cache.has(role.id)) {
                                        rolesReplaced.set(role.id, role);
                                    }
                            ...
                            const filteredRoles = fetchedRoles.filter((role) => selectedRoles.includes(role.name.toLowerCase()));
                                ...
                                (() => {
                                    selectedRoles.forEach((selectedRoleName) => {
                                        ...
                                        filteredRoles.forEach((fetchedRole) => {
                                            if (selectedRoleName === fetchedRole.name.toLowerCase())
                                                found = true;
                                        });
                                        if (!found) {
                        ...
                        Please let my creator (${userMention(process.env.CREATOR_ID)}) know of this error!
                        ...
                        `,
                            ...
                            filteredRoles.forEach((role) => {
                                if (memberRoles.cache.has(role.id)) {
                                    console.log(`${interaction.user.tag} already has the role ${role.name}, marking for removal...`);
                                    diffMessage += `- Removing: ${role.name}\n`;
                                    rolesMarkedForRemoval.set(role.id, role);
                                }
                                else {
                                    console.log(`${interaction.user.tag} does not have the role ${role.name}, marking for addition...`);
                            ...
                            if (rolesMarkedForRemoval.size === 0 && rolesMarkedForAddition.size !== 0) {
                                ...
                                if (['minor', 'boomer'].includes(rolesMarkedForAddition.first().name.toLowerCase())) {
                                    roleType = 'age';
                                }
                                else if ([
                                    'woman lover',
                                    ...
                                    console.log(`the role to remove didn't match any type???? got ${rolesMarkedForAddition
                                        .first()
                                        .name.toLowerCase()}`);
                                }
                                const roleTypes = ['age', 'sexuality', 'pronouns', 'color', 'pings'];
                                ...
                                if (rolesMarkedForAddition.size === 1) {
                                    rolesAddedEmbed.setDescription(stripIndent `
                      The role ${roleMention(rolesMarkedForAddition.firstKey())} was successfully added!
                      `);
                                    if (rolesReplaced.size > 0) {
                                        try {
                                            ...
                                            if (rolesReplaced.size === 1) {
                                                rolesAddedEmbed.setDescription(stripIndent `
                        The role ${roleMention(rolesReplaced.firstKey())} was replaced with ${roleMention(rolesMarkedForAddition.firstKey())} successfully!
                      `);
                                            }
                                            else {
                  ...
                  You have selected the role ${roleMention(rolesMarkedForRemoval.firstKey())}, which you already have.
                  Would you like to remove it?
                  `);
                                    buttonRow.setComponents([
                                        new ButtonBuilder()
                                            .setCustomId(`${mainAction}|${secondaryAction}|removeRole|${rolesMarkedForRemoval.firstKey()}`)
                                            .setLabel('Remove Role!')
                                            .setStyle(ButtonStyle.Primary),
                                        new ButtonBuilder()
                                            .setCustomId(`${mainAction}|${secondaryAction}|selectRoleBasedCategory|${anythingElse[0]}`)
                  ...
                  `);
                                ...
                                buttonRow.setComponents([
                                    new ButtonBuilder()
                                        .setCustomId(stripIndent `${mainAction}|${secondaryAction}|updateRoles|${roleNamesToAdd.concat(roleNamesToRemove)}`)
