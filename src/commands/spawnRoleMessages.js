...
export const data = new SlashCommandBuilder()
    ...
    .setDescription('Creates (or replaces) the messages for the role selections')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageRoles);
export async function execute(interaction) {
    try {
        const { client, channel } = interaction;
        if (!['1072045353169334373', '1015749480840167435'].includes(interaction.channelId)) {
        ...
        const roleMessages = rawMessages.filter((message) => message.author.id === client.user.id &&
            message.embeds.length !== 0 &&
            message.embeds[0].title === 'User Roles');
        if (roleMessages.size !== 0) {
            const startMessageEmbed = EmbedBuilder.from(roleMessages.first().embeds[0]);
            const rawEditRolesButton = roleMessages
                .first()
                .components[0].components.filter((component) => component.type === ComponentType.Button)[0];
            const editRolesButton = new ActionRowBuilder().addComponents([
            ...
            const startMessageEmbed = new EmbedBuilder()
                ...
                .setTitle('User Roles').setDescription(stripIndent `
